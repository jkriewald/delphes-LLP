...
module GraphDisplacedVertexFinder GraphDisplacedVertexFinder {
  ...
  # Output arrays
  set VertexOutputArray         DisplacedVertices
  set PVOutputArray             PrimaryVertices
  set EFlowOutputArray          displacedeflowTracks
  

  
  # --- Track preselection ---
  set MinTrackIPSig   3.0 ;# Impact parameter Significance
  set MinD0           0.0 ;# D0 (mm)
  set MinZ0           0.0 ;# Z0 (mm)
  set MinTrackPT      0.1 ;# GeV

  # --- Primary vertex ---
  set BeamSpotSigmaX  0.01 ;# transverse beamspot spread mm
  set BeamSpotSigmaY  0.01 ;# transverse beamspot spread mm
  set BeamSpotSigmaZ  1.0  ;# longitudinal beamspot spread mm
  set PVChi2NDFMax    9.21 ;# max chi2/ndf for PV fit 
  set UsePVCut        true ;# use PV association cut
  set PVCutChi2       9.21 ;# maximum chi2 distance for PV association

  # --- Clustering / seeding ---
  set SeedSelector     2   ;# -1: no seed selection, 0:min pT, 1:max IP sig, 2:lepton
  set MinSeedPT        0.1 ;# GeV
  set MinSeedIPSig     5.0 ;# Impact parameter Significance for seed track
  set MinTracks        3   ;# minimum number of track per connected component to be fitted
  set kNN             -1   ;# -1: no kNN prune
  set MinSupport       1   ;# triplet support, 0: no pruning, 1: at least one witness track, 2: at least two, etc...
  set UsePairGuard     true ;# use first hit guard to prune pairs
  set PruneBridges     true ;# prune bridges between sub graphs
  set BridgeCut         3.0  ;# stronger chi2 cut to keep strong bridges

  # --- Pair/triplet gates & weights ---
  set chi2PairCut      9.21 ;# maximum chi2/ndf for pair fit
  set chi2TripletCut   9.21 ;# maximum chi2/ndf for triplet fit
  set weightCut        0.5  ;# minimum weight for track to stay associated in fit
  set Chi2NDFMax       9.21 ;# maximum chi2/ndf for fit
  set UseTiming        true ;# Use timing compatibility in the pair graph       
  set TimeGate         9.21 ;# chi^2 gate for the time compatibility
  set TimeVarianceCap  4.0  ;# (mm^2) maximum additional variance to consider a track well-timed

  # --- Fitter / annealing ---
  set chi2_0        9.21 ;# sigmoid mid
  set beta          2.0  ;# sigmoid width

  # --- Assign leftovers & misc ---
  set Chi2AssociationMax   9.21 ;# maximum chi2 distance for track association
  set CorrectMomenta       true ;# correct momenta of tracks and refit perigee parameters
  set Verbose             false ;# some log messages

  # --- Timing Layer Geometry (TimeSmearing assumes it at the edge of the magnetic field coverage so that's what we'll stick to) ---
  set TimingR $R 
  set TimingZ $HL
}


module Merger DisplacedEFlowMerger {
  add InputArray GraphDisplacedVertexFinder/displacedeflowTracks
  add InputArray NeutralEFlowMerger/neutraleflow
  set OutputArray mergedeflow
}

module PdgCodeFilter ElectronFilter2 {
  set InputArray DisplacedEFlowMerger/mergedeflow
  set OutputArray electrons
  set Invert true
  add PdgCode {11}
  add PdgCode {-11}
...
module PdgCodeFilter MuonFilter2 {
  set InputArray DisplacedEFlowMerger/mergedeflow
  set OutputArray muons
  set Invert true
  add PdgCode {13}
...
module Isolation ElectronIsolation2 {
  set CandidateInputArray ElectronFilter2/electrons
  set IsolationInputArray GraphDisplacedVertexFinder/displacedeflowTracks
...
module Isolation MuonIsolation2 {
  set CandidateInputArray MuonFilter2/muons
  set IsolationInputArray GraphDisplacedVertexFinder/displacedeflowTracks
...
module IdenticalObjectRemover IdenticalObjectRemover {
 set InputArray DisplacedEFlowMerger/mergedeflow

 set OutputArray cleanedeflow

 add RemoveArray ElectronIsolation2/electrons
 add RemoveArray MuonIsolation2/muons

}

module FastJetFinder DJetFinder {
#  set InputArray Calorimeter/towers
  set InputArray IdenticalObjectRemover/cleanedeflow

  set OutputArray djets
...
module LLPReconstruction LLPReconstruction {

  set InputVertices GraphDisplacedVertexFinder/DisplacedVertices
  set InputJets DJetFinder/djets

  set OutputLLPs LLPs
  set OutputJetClones djets
  set OutputElectrons delectrons
  set OutputMuons dmuons

  set cosThetaCut 0.
  set MomentumThreshold 0.8
  set MaxDeltaR 0.3
  set PTAlpha 1.0
  set OverlapThreshold 0.
  set MinLeptonPT 0.
  set Verbose false
  }
...
module TreeWriter TreeWriter {
    ...
    add Branch ElectronIsolation2/electrons Electron Electron
    add Branch MuonIsolation2/muons Muon Muon
    add Branch PhotonEfficiency/photons Photon Photon

    add Branch JetEnergyScale/jets Jet Jet
    add Branch MissingET/momentum MissingET MissingET

    add Branch GraphDisplacedVertexFinder/PrimaryVertices PrimaryVertex DisplacedVertex
    add Branch GraphDisplacedVertexFinder/DisplacedVertices GraphDisplacedVertex DisplacedVertex
    add Branch DJetFinder/djets DJet Jet
    add Branch LLPReconstruction/djets DisplacedJet DisplacedJet
    add Branch LLPReconstruction/delectrons DisplacedElectron DisplacedLepton
    add Branch LLPReconstruction/dmuons DisplacedMuon DisplacedLepton
    add Branch LLPReconstruction/LLPs LLPs LLPCandidate
    add Branch GraphDisplacedVertexFinder/displacedeflowTracks DEFlowTrack Track
